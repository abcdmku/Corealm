...
Corealm publishes four GLBs and one texture atlas derived from three Unity Asset Store packages. These outputs are not CC0. The local package files and their hashes identify the exact inputs used on this machine, but they do not prove account ownership or a transferable licence. The project owner must confirm that the shipping project has the required Asset Store entitlement.
...
`tools/import-unity-magic-assets.ps1` checks the first two package hashes before it opens Unity. `tools/import-unity-magic-assets.ts` does not read either source package. It audits the four exported GLBs and enforces their recorded output hashes. The Hovl package hash is a reproduction record; `tools/build-assets.ts --verify` enforces the committed atlas hash recorded in `manifest.json`.
