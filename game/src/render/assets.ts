...
export class AssetRegistry {
   ...
   *
   * This is how a mesh with no file behind it reaches a consumer that only knows how to `load`.
   * `render/proceduralGear.ts` builds the four staves out of primitives because the 213-asset
   * library has no staff mesh at all; once they are registered here, `load("proc_staff_worn")`
   * resolves, and `characterRig.attachBoneSlot` — which does nothing but `assets.load(assetId)`,
   * clone, socket and parent — needs no change and no knowledge that the mesh was generated.
   *
   * DELIBERATELY NOT A MANIFEST ENTRY, which was the other way to do this. The manifest is
   * generated by tools/build-assets.ts from real files on disk: it carries `file`, `pack`, `bytes`,
   * `size`, `base`, `materials` and a licence-bearing `pack` id, every one of them measured by
   * opening the GLB. A hand-written row for a mesh with no GLB would be a lie in a generated file,
   * and every consumer of those fields — `baseY` for ground alignment, `assetSize`, the pack
   * licence report, the build tool's own rewrite of the file — would need a permanent special case
   * for it. The cache is the honest place: `load` is a promise for a scene graph, and a built graph
   * is one.
   *
   * Callers keep ownership of the geometry and materials. Nothing in this class disposes a cached
   * group, built or loaded.
   */
