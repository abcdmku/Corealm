...
export class EntityViews {
  ...
  private syncOne(entity: SemanticEntity): void {
    ...
    const groupKey = `${character?.key ?? view.assetId}|${view.depletedAssetId ?? "-"}|${this.groupTier(entity.archetype, tier, view.assetId, character)}|${entity.archetype}|${clip}`;
  ...
  private ensureGroup(
    ...
    group.live = this.buildDraws(group.liveParts);
  ...
  private ensureSpent(group: InstanceGroup): void {
    ...
    group.spent = this.buildDraws(group.spentParts);
  }
  ...
  private ensureMoving(group: InstanceGroup): void {
    ...
    group.moving = this.buildDraws(group.movingParts);
  }
   ...
   */
  private batchFor(part: SourcePart): Batch | null {
    const position = part.geometry.getAttribute("position");
    if (!position) return null;
    const key = `${materialBatchKey(part.material)}||${attributeSignature(part.geometry)}`;
  ...
  private buildDraws(parts: readonly SourcePart[]): PartDraw[] {
